...
async function UpcomingApi() {
    ...
    try {
        ...
        const upComingEvents = res.response.filter(
            (event) => event.date > fechaActual
        );

        ApiGlobal = upComingEvents;

        tarjetasUpcoming(upComingEvents);
        categoriasUnicas(upComingEvents)
...
function resetTemplateCardUpcoming() {
    document.querySelector("#TemplateCardUpcoming").innerHTML = ''
}

function SearchNotFoundTemplateCardUpcoming() {
    let templateCard = document.querySelector("#TemplateCardUpcoming");
...
function newCard(evento) {
    ...
    divForCard.innerHTML = `<div class="card h-100" style="max-width: 400px;">
        ...
        <ul class="list-group list-group-flush">
          ...
          <li class="list-group-item"><i class="bi bi-people-fill me-2"></i>${evento.estimate}</li>
...
function searchCards() {
    ...
    if (CheckedCategory.length > 0) {
        ...
        if (filtrado.length > 0) {
            ...
            resetTemplateCardUpcoming();
            SearchNotFoundTemplateCardUpcoming();
        }
